...
public class Delivery {
    ...
    public int newCustomer(String name, String address, String phone, String email) 
        ...
        Customer customer = new Customer(name, address, phone, email, customerID);
    ...
    public List<String> findItem(String search) {
    	return menuItems.values().stream()
    					  .filter(mI -> mI.getDescription()
    							  	      .toLowerCase().contains(search.toLowerCase()))
    					  .sorted(comparing(MenuItem::getCategory)
    ...
    public int newOrder(int customerId) {
    	orders.put(orderID, new Order(customerId));
        return orderID++;
    }
    ...
    public int addItem(int orderId, String search, int qty) throws DeliveryException {
    	...
    	String item = findItem(search).stream().findFirst().orElse(null);
    	// findItem ha il formato strano: [CATEGORIA] DESCRIZIONE : PREZZO
    	item = item.replaceAll("\\[.*\\] ", "");
    	item = item.replaceAll(" :.*", "");
//    	System.out.println("Item: "+item);
    	// OSS: in realta' "search" era la descrizione ahahah
    	orders.get(orderId).putItem(menuItems.get(item), qty);
        
    	return orders.get(orderId).getItemQuantity(menuItems.get(item));
    }
    ...
    public List<String> showOrder(int orderId) throws DeliveryException {
    	if (!orders.containsKey(orderId))
    		throw new DeliveryException();
    	return orders.get(orderId).getMenuItems();
    }
    ...
    public double totalOrder(int orderId) throws DeliveryException {
    	if (!orders.containsKey(orderId))
      	...
      	return orders.get(orderId).getTotal();
    }
    ...
    public int confirm(int orderId) throws DeliveryException {
    	if (!orders.containsKey(orderId)  ||  
    		orders.get(orderId).getStatus() != OrderStatus.NEW)
			throw new DeliveryException();
    	
    	orders.get(orderId).setStatus(OrderStatus.CONFIRMED);
    	return orders.get(orderId).getShippingTime();
    }
    ...
    public int start(int orderId) throws DeliveryException {
    	if (!orders.containsKey(orderId)  ||  
        		orders.get(orderId).getStatus() != OrderStatus.CONFIRMED)
    			throw new DeliveryException();
    	orders.get(orderId).setStatus(OrderStatus.PREPARATION);
    	return orders.get(orderId).getShippingTime()-5;  // 5 e' il ritardo 
    }
    ...
    public int deliver(int orderId) throws DeliveryException {
    	if (!orders.containsKey(orderId)  ||  
        		orders.get(orderId).getStatus() != OrderStatus.PREPARATION)
    			throw new DeliveryException();
    ...
    public void complete(int orderId) throws DeliveryException {
    	if (!orders.containsKey(orderId)  ||  
        		orders.get(orderId).getStatus() != OrderStatus.ON_DELIVERY)
    			throw new DeliveryException();
    ...
    public SortedMap<Double,List<String>> bestCustomers(){
    	return orders.values().stream()
    			...
    			.collect(groupingBy(order -> totalCustomer(order.getCustomerId()),
//    								Order::getTotal,  /* errato perche' un cliente puo' avere piu' ordini e
//    												   * un totale pari alla somma dei totali */
    								() -> new TreeMap<Double,List<String>>(reverseOrder()),
					    			mapping(order -> customers.get(order.getCustomerId()).toString(), 
					    					collectingAndThen(toList(), 
					    					       			  l -> l.stream()
					    							 			    .sorted()
					    							 			    // possono esserci piu' ordini da un cliente
					    							 			    .distinct()
					    							 				.collect(toList())
					    							 		  )
					    					 )
